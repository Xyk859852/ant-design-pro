...
export const errorConfig: RequestConfig = {
  ...
  errorConfig: {
    ...
    errorHandler: (error: any, opts: any) => {
      ...
      if (error.name === 'BizError') {
        const errorInfo: ResponseStructure | undefined = error.info.data;
        if (errorInfo) {
  ...
  requestInterceptors: [
    (config: RequestOptions) => {
      ...
      const authHeader = { 'Access-Token': 'Bearer ' + token };
  ...
  responseInterceptors: [
    (response) => {
      // 拦截响应数据，进行个性化处理
      const { data, code, message, showType } = response as unknown as ResponseStructure;
      if (data?.code != 0) {
